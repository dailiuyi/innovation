<template>
  <el-drawer :model-value="!!sceneId" title="内容版本草稿" size="90%" :close-on-click-modal="false" :before-close="close">
    <p>文件仅供管理员入库管理；草稿未发布，不能供客户端加载。AAR 请登记为客户端集成库。</p>
    ...
    <el-table :data="drafts" highlight-current-row :current-row-key="selected?.id" row-key="id">
      ...
      <el-table-column label="状态" width="90"><template #default>草稿</template></el-table-column>
      <el-table-column label="操作" width="170"><template #default="{row}">
        ...
        <el-button link type="danger" :disabled="busy || removing" @click="removeDraft(row)">删除</el-button>
    ...
    <section v-if="selected" class="draft-files">
      ...
      <el-table :data="files" v-loading="loadingFiles">
        ...
        <el-table-column label="操作" width="190"><template #default="{row}"><el-button v-if="['PENDING','FAILED'].includes(row.status)" link type="primary" :disabled="busy || removing" @click="choose(row)">重新选择原文件</el-button><el-button link type="danger" :disabled="busy || removing || ['UPLOADING','DELETING'].includes(row.status)" @click="removeFile(row)">{{row.status==='DELETE_FAILED'?'重试删除':'删除'}}</el-button></template></el-table-column>
      </el-table>
...
async function loadDrafts(){
  ...
  drafts.value=r.items;draftTotal.value=r.total
}
...
async function refresh(){try{error.value='';await loadDrafts();if(selected.value)await loadFiles()}catch(e){error.value=message(e)}}
...
watch(()=>props.sceneId,async id=>{++selectionRequest;++fileRequest;++draftRequest;selected.value=null;files.value=[];drafts.value=[];fileTotal.value=0;draftTotal.value=0;loadingFiles.value=false;error.value='';draftPage.value=1;if(!id)return;try{maxBytes.value=(await request.get('/api/v1/drafts/config')).maxBytes;if(id!==props.sceneId)return;await loadDrafts();if(id===props.sceneId&&route.query.draftId)await selectDraft(String(route.query.draftId))}catch(e){if(id===props.sceneId)error.value=message(e)}},{immediate:true})
