...
ZIP 的请求结果和构建结果是两件事。准备请求遇到网关 504、客户端超时或连接中断时只表示结果未确认，页面提示“请求等待超时，ZIP 构建结果尚未确认，后端可能仍在处理。请稍后重试。”，不显示“ZIP 构建失败”，也不断言已成功；后端明确返回失败时显示后端错误；ZIP 已准备好而内容下载出错时单独提示“ZIP 下载失败，请重试”。异常后按钮恢复可用，用户手动重试；已经生成并可用的 ZIP 会被复用，不因一次失败重做打包。
...
验收覆盖双文件关联、刷新持久化、正式存储重新读取、大小/摘要错误、重复及并发请求、真实连接中断、上传中强制结束服务、重启恢复、草稿整份删除确认/取消、空草稿拒绝发布、失败文件拒绝发布和下载、文件夹替换确认/取消、清单与 ZIP 下载及 Range、发布/替换指针、已发布文件冻结、当前发布禁止删除、说明仍可修改、发布确认取消、匿名和非管理员拒绝，以及操作审计归属。浏览器在真实 ZIP 下载成功后继续模拟网关 504、准备请求中断、后端明确失败和下载阶段失败，检查提示文案、按钮恢复和重试复用同一份包。另有`LocalArtifactStorageTest`、`LocalZipExportStoreTest`、`RelativePathTest`、`ZipWriteBenchmarkTest` 和前后端构建。
