...
本阶段已提供四个服务的 Docker Compose 配置：Nginx、Spring Boot、PostgreSQL 17、Redis。业务能力仍为现有账号、场景与审计。算法在工作站生产成品，未来由管理后台上传；客户端负责加载和渲染。

`ArtifactStorage` 和 `LocalArtifactStorage` 已实现本地文件暂存、大小/SHA256 校验、不可覆盖确认、读取、校验及文件盘点。组件没有 HTTP 入口，没有业务表，也没有公开文件目录。文件确认入库不等于业务发布。

真实成品及加载方式尚未提供，因此本轮不实现上传页面、文件登记、版本关联、发布回滚、客户端清单和下载授权。旧候选契约和七表脚本不会参与容器初始化；昼夜、AR 坐标、文件数量和平台格式仍待重新确认。
...
`AR_STORAGE_MAX_BYTES` 是必填的正整数，真实样包测量后再设实际值。现有框架的 multipart/Nginx 20m 配置不代表产物上传契约；产物入口尚未开放。
